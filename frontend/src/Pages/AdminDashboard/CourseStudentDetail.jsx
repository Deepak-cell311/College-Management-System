...
import { ArrowLeft, Calendar, GraduationCap, PieChart, CheckCircle, XCircle } from 'lucide-react';
...
const CourseStudentDetail = () => {
    ...
    useEffect(() => {
        if (studentTodo) fetchStudentData();
    }, [studentTodo]);

    if (!studentTodo) return <div className="min-h-screen bg-gray-900 text-white flex justify-center items-center">No student selected</div>;
                ...
                {loading ? (
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <h1 className="text-3xl font-bold text-gray-100">{studentData[0]?.name}</h1>
                                <p className="text-gray-400 mt-1">Roll Number: {studentData[0]?.rollNum}</p>
                            </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div className="lg:col-span-1 bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg">
                                <CanvasJSChart options={activeTab === 'attendance' ? attendanceChartOptions : marksChartOptions} />
                            </div>
                            ...
                            <div className="lg:col-span-2 bg-gray-800 rounded-xl border border-gray-700 overflow-hidden shadow-lg">
                                <div className="p-4 border-b border-gray-700">
                                    <h2 className="text-lg font-semibold flex items-center gap-2">
                                        {activeTab === 'attendance' ? <PieChart className="w-5 h-5 text-blue-400" /> : <GraduationCap className="w-5 h-5 text-purple-400" />}
                                        {activeTab === 'attendance' ? 'Attendance History' : 'Exam Results'}
                                    </h2>
                                </div>
                                        ...
                                        <tbody className="divide-y divide-gray-700">
                                            {activeTab === 'attendance' ? (
                                                ...
                                                studentData[0].examResult.map((result, index) => (
                                                    <tr key={index} className="hover:bg-gray-700/50 transition-colors">
                                                        <td className="px-6 py-4 text-gray-300 font-medium">{result.subName}</td>
                                                        <td className="px-6 py-4 text-gray-300">
                                                            <div className="flex items-center gap-3">
                                                                <div className="flex-1 h-2 bg-gray-700 rounded-full max-w-[100px]">
                                                                    <div
                                                                        className="h-full bg-purple-500 rounded-full"
                                                                        style={{ width: `${Math.min(result.marksObtained, 100)}%` }}
                                                                    />
                                                                </div>
                                                                <span className="text-sm font-medium">{result.marksObtained}</span>
                                                            </div>
                                                        </td>
                                                    </tr>
                                                ))
